...
function ProductCard({ producto }) {
  ...
  const waText = encodeURIComponent(`Hola ${marca.nombre}, quiero consultar sobre: ${producto.nombre}`);
  const waLink = `https://wa.me/${marca.whatsapp}?text=${waText}`;

  return (
    <article
      ref={ref}
      className={`bg-white border border-brand-green/10 rounded-sm overflow-hidden hover:shadow-md hover:border-brand-green/30 transition-all duration-300 flex flex-col group ${inView ? 'animate-fade-up' : 'opacity-0'}`}
    >
      ...
      <div className="p-5 flex flex-col flex-1">
        <h3 className="font-bold text-brand-green-dark text-base uppercase tracking-wide mb-1">{producto.nombre}</h3>
        {producto.spec && (
          <p className="font-mono text-xs text-brand-green/80 mb-2">{producto.spec}</p>
        )}
        <p className="text-brand-text/60 text-sm flex-1 mb-4 leading-relaxed">{producto.descripcion}</p>
        <div className="flex items-center justify-between gap-3 mt-auto pt-3 border-t border-brand-green/10">
          <span className="font-mono font-medium text-brand-green-dark text-base">{producto.precio}</span>
          <a
            href={waLink}
            target="_blank"

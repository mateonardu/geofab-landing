...
export default function ProductoAntimaleza() {
  const producto = productos.find(p => p.slug === 'tela-antimaleza');
  const [selected, setSelected] = useState(producto.variantes[0]);

  const waLink = `https://wa.me/${marca.whatsapp}?text=${encodeURIComponent(
    `Hola ${marca.nombre}, me interesa la tela antimaleza de ${selected.id} grs`
  )}`;
          ...
          <div className="w-full md:w-1/2 relative z-10 pt-14 pb-8 md:pt-16">
            ...
            <div>
              ...
              <div className="border border-brand-green/10 rounded-sm overflow-hidden">
                {selected.fichaTecnica.map((f, i) => (
                  <div
                    key={f.clave}
                    className={`flex gap-4 px-4 py-2.5 text-sm ${i % 2 === 0 ? 'bg-white' : 'bg-brand-cream/30'}`}
                  >
      ...
      <div className="bg-brand-green-dark py-16 px-4 mt-4">
        <div className="max-w-5xl mx-auto">
          ...
          <div className="text-center mt-12">
            ...
            >
              <WaIcon />
              Consultar {selected.id} grs por WhatsApp
            </a>
          </div>
        </div>
      </div>
